...
//var iohook = require("/usr/meanOs/systemX/Shared/CoreWindow/js/iohook");
...
App.addNewInstanceTab = function (files,autoSwitch,noLoad) {
	...
	if (appLocation.indexOf("mean.files.app") != -1)
...
var defaultTheme = require('/usr/meanOs/systemX/Shared/CoreWindow/js/photo_editor_element/theme/white-theme.js');
...
function checkIfAnotherWindowExistBehindWIndow() {
	if ((currentlyProcessingIDPos < winIdsToProcess.length) && keepChecking) {
		executeNativeCommand('/usr/meanOs/systemX/mean.explorebar/js/windowDimensionsById.sh '+winIdsToProcess[currentlyProcessingIDPos], function(data, err) {
			if (!err) {
...
win.on('focus', function() {
	if (!Appready) {
		...
		if (appLocation.indexOf("mean.itai.app") != -1) {
...
const appCommsChannel = new BroadcastChannel("meanOsApp"+win.cWindow.id); //Comms channel between process manager and this App
...
//const appCommsChannelTemp = new BroadcastChannel("meanOsAppListerning");
/*
appCommsChannelTemp.onmessage = function (ev) {
	...
	//const appCommsChannel = new BroadcastChannel("meanOsApp"+ev.data.id);
...
} else { //if (appLocation.indexOf("mean.itai.app") == -1)
...
if (App.argv != null) {
	if (App.argv[0] == "--screnshot-extern\0" && ev.data.appLocation.indexOf("mean.photos.app") != -1) {
...
console.log("lets go lol");
	...
	if (ev.data.appLocation.indexOf("mean.files.app") != -1) {
...
$("#outerBody").addClass("noMargins");
	$("#body_settings").removeClass("zeroOpacity");
		...
		if (ev.data.appLocation.indexOf("mean.welcome.app") != -1) {
		...
		if (ev.data.appLocation.indexOf("mean.photos.app") != -1) {
		...
		if (ev.data.appLocation.indexOf("mean.tips.app") != -1) {
